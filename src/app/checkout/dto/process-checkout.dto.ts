...
export class CheckoutCardTokenDto {
  @IsString()
  @IsNotEmpty()
  @ApiProperty({ description: 'Cielo card token', example: 'token-uuid' })
  token: string;
  ...
  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ description: 'Cardholder name', example: 'JOÃO SILVA' })
  holder?: string;

  @IsOptional()
  @IsString()
  @Matches(/^\d{3,4}$/, { message: 'Security code must be 3 or 4 digits' })
  @ApiPropertyOptional({ description: 'Security code (CVV)', example: '123' })
  securityCode?: string;
...
export class CheckoutDebit3dsDto {
  @IsString()
  @IsNotEmpty()
  @ApiProperty({ description: 'Cavv retornado pelo 3DS', example: '00000000' })
  cavv: string;

  @IsString()
  @IsNotEmpty()
  @ApiProperty({ description: 'Eci retornado pelo 3DS', example: '05' })
  eci: string;

  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ description: 'Xid retornado pelo 3DS', example: '0000' })
  xid?: string;

  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ description: 'Versão do protocolo 3DS', example: '2.2.0' })
  version?: string;

  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ description: 'ReferenceId retornado pelo 3DS', example: '00000000-0000-0000-0000-00004e6' })
  referenceId?: string;
}

export class CheckoutDebitCardDto {
  @IsString()
  @ApiProperty({ description: 'Nome do portador', example: 'JOÃO SILVA' })
  name: string;

  @IsString()
  @ApiProperty({ description: 'Número do cartão de débito', example: '4000000000000001' })
  number: string;

  @IsString()
  @ApiProperty({ description: 'Validade (MM/YY)', example: '12/25' })
  expiry: string;
...
export class CheckoutPaymentDto {
  ...
  @ApiPropertyOptional({ description: 'Raw credit card data (use cardToken instead when possible)', type: CheckoutCardDto })
  card?: CheckoutCardDto;

  @IsOptional()
  @ValidateNested()
  @Type(() => CheckoutCardTokenDto)
  @ApiPropertyOptional({ description: 'Pre-tokenized card from Cielo (preferred over raw card data)', type: CheckoutCardTokenDto })
  cardToken?: CheckoutCardTokenDto;

  @IsOptional()
  @ValidateNested()
  @Type(() => CheckoutDebitCardDto)
  @ApiPropertyOptional({ description: 'Dados do cartão de débito (obrigatório para DEBIT_CARD)', type: CheckoutDebitCardDto })
  debitCard?: CheckoutDebitCardDto;

  @IsOptional()
  @ValidateNested()
  @Type(() => CheckoutDebit3dsDto)
  @ApiPropertyOptional({ description: 'Dados de autenticação 3DS para débito (pós-autenticação externa)', type: CheckoutDebit3dsDto })
  threeds?: CheckoutDebit3dsDto;

  // PIX and Boleto: gerado pelo gateway
}
